...
Additive over the weight table below, clamped to [0, 1], then banded. Combos ("a
password form *on a young domain*") add on top of their constituents because the
combination is the tell, not either half alone.

**This is a structural prior, not a reputation verdict.** The scan path collects no
reputation data, so the two signals that would dominate a real verdict - Google Web
Risk and VirusTotal - are absent, and the bands are calibrated accordingly. Present it
to analysts as a heuristic; adding OTX or VirusTotal would earn a stronger claim.

**Threshold calibration.** Both bands are anchored to the case table in
...
* a single ``untrusted_tls`` (0.25) must clear the suspicious band while a lone
  ``password_form`` (0.15) must not, pinning ``SUSPICIOUS_THRESHOLD`` to (0.15, 0.25];
* a payload download on a young domain with a zero security-header score
  (0.30 + 0.20 + 0.20 combo + 0.10 = 0.80) must land in "suspicious", not "malicious",
  pinning ``MALICIOUS_THRESHOLD`` above 0.80.

``SUSPICIOUS_THRESHOLD`` sits at the *top* of its permitted range deliberately. At the
previous 0.20, the two weakest and most common signals on the open web - a thin
security-header score (0.10) and a freshly minted (i.e. Let's Encrypt) certificate (0.10)
- summed to exactly the floor, so a large fraction of ordinary, harmless sites read as
"suspicious". 0.25 requires either one genuinely notable signal (``untrusted_tls``) or a
combination including something stronger than "modern free TLS".
"""
...
# This order is also the order signals are reported in.
WEIGHTS = {
...
# Word boundaries, not substrings: "chase" is in "purchase", "apple" is in
# "applesauce", and every page containing either read as suspicious.
...
def _parse_date(value):
    """The assorted date shapes WHOIS and OpenSSL hand us. None on failure."""
    if not value:
        return None
...
def _brand_domain_mismatch(scan, has_password: bool) -> bool:
    """A brand named on a *credential-soliciting* page whose token is absent from the
    page's *registrable* domain. Three constraints, all learned the hard way:

    1. **Word boundaries, not substrings** - a bare test scored "Complete your purchase
       today" as brand impersonation.
    2. **A brand mention alone is never a signal.** Naming a brand is what ordinary pages
       do all day; naming one next to a password box is the tell. Hence the
       ``has_password`` gate, with the heavy lifting left to the combo weight.
    3. Matching the *registrable* domain, not the full host: that is what catches
       ``microsoft-login.verify-account.test``, which hides "microsoft" in a subdomain
       label, while ``login.microsoftonline.com`` keeps it in its registrable domain.
    """
    if not has_password:
        return False
...
def score(scan: dict) -> dict:
    """``level`` is ``unknown`` when the scan gathered no usable evidence - never a
    false ``benign``.
    """
    page = scan.get('page') or {}
    tls = scan.get('tls') or {}
