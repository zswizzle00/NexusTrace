...
Additive weights over named boolean signals, clamped to [0, 1], then banded. Combos add
on top of their constituents because the combination is the tell, not either half: an
obfuscated string-execution primitive is a code smell, the same primitive *next to a
download cradle* is a stager.

**A structural prior over static observations, not a reputation verdict.** The only
non-structural input is `known_malware_hash`. Weights are per-signal booleans, never
`weight * occurrence_count` - a packed binary that mentions `curl` forty times is not
forty times more suspicious, and an unbounded count-scaled total cannot be banded.

Derez's `apple_analyzer._RISK_WEIGHTS` supplied the signal vocabulary and none of its
model; its `unsigned`, `dangerous_entitlement`, `suspicious_dylib` and `yara_match`
signals are deliberately **absent** rather than faked, because this record carries no
code signature, entitlement list, dylib table or YARA result, and inventing a value for
a field nothing populates would score absence.

**Absence never scores, and never reads as clean.** `no_reputation_data`,
`analysis_truncated` and `analysis_error` are weighted at exactly 0.0 - reported so the
analyst can see *why* a verdict is thin, moving the total by nothing. `benign`
additionally requires that a reputation source actually answered: static inspection alone
can raise a verdict but can never clear one, so a file whose every lookup was skipped for
want of an API key is `unknown`, not `benign` (MalwareBazaar and ThreatFox need no key, so
`benign` stays reachable in a configured deployment). Unlike `email_rules`, `unknown`
keeps its score and signals: weak-but-real observations plus "we could not check
reputation" is more honest than a zeroed report.

**The ambient-floor property.** Two profiles describe ordinary, harmless files, and both
must stay below `SUSPICIOUS_THRESHOLD`:

* *A legitimate packed installer* - executable, high-entropy, fetches over HTTP, long
  base64 run in its resources - **0.20**. A signed `setup.exe` must not read as
  suspicious merely for being a packed executable; the engine's most important
  false-positive anchor.
* *An ordinary per-user shortcut* (VS Code, Teams, Chrome all install to
  `%LOCALAPPDATA%`) - executable-by-extension, user-writable path outside Program
  Files, names a UNC path or URL, size inconsistency, requests elevation - **0.25**.

Stated per named profile, not as a global cap over the 0.05 tier (all eight ambient
signals sum to 0.40): a file would have to be simultaneously a `.lnk`, high-entropy and
carrying both a base64 blob and fetch-API strings to collect them all, and a base64 blob
in a shortcut's arguments fires `lnk_encoded_command` (0.30, over the floor by itself),
so that composite is never an ambient-only file.
...
* a lone `embedded_executable_at_offset` (0.30) - a PE appended to a file that is not
  one - must clear `suspicious`, which puts it at or below 0.30.

  Those two together pin `SUSPICIOUS_THRESHOLD` to exactly **0.30**. Moving any 0.05
  ambient weight, or `embedded_executable_at_offset`, re-opens the derivation.

* a PDF carrying an appended PE (0.30 + 0.25 + 0.05 = 0.60) must stay `suspicious`,
  and a `.pdf`-named PE (0.45 + 0.05 + 0.05 = 0.55) must too, which puts
  `MALICIOUS_THRESHOLD` above 0.60;
* a packed PE carrying C2 framework, sandbox-evasion and credential-dumper strings
  (0.25 + 0.25 + 0.30 + 0.10 = 0.90) must reach `malicious`, putting it at or below
  0.90.

  Within (0.60, 0.90], **0.85** is chosen so that `known_malware_hash` - weighted at
  exactly `MALICIOUS_THRESHOLD`, mirroring `email_rules.attachment_known_malware` -
  reaches `malicious` on its own, including for a malicious *document* that earns no
  structural weight at all.

Every field is read defensively: the record is assembled from attacker-controlled
bytes by helpers documented to degrade rather than raise, so it may be partial, and a
malformed record must produce a verdict dict rather than 500 an upload route.
"""
...
WEIGHTS = {
    ...
    # Strong: the file lies about what it is, carries a second program, or names a
    # primitive with no benign reading.
    ...
    'reverse_shell': 0.30,
    # Moderate: each has a legitimate reading often enough that none may reach a
    # verdict without company.
    'anti_analysis': 0.25,
    'c2_artifact': 0.25,
    ...
    'crypto_address': 0.10,
    'persistence': 0.10,
    # Ambient - see the ambient-floor property above.
    'base64_blob': 0.05,
...
# more than recall. Below MIN_ENTROPY_SIZE the measurement is meaningless - a
# 300-byte file cannot distribute 256 symbols evenly enough to say anything.
HIGH_ENTROPY_MIN = 7.5
MIN_ENTROPY_SIZE = 4096

# `extract_strings` already caps itself at 2000 x 512 chars (~1 MB); scanning a
# megabyte with ~15 compiled alternations is wasteful when the strings are dense.
MAX_STRINGS_TEXT = 400_000

# Media and text/office formats only - never an extension that legitimately holds code.
...
# LNK on top of file_inspect.EXECUTABLE_MAGIC because a shortcut named `report.pdf`
# is exactly the same lie as a PE named `report.pdf`; file_inspect keeps its own set
# narrow on purpose, because widening it would change persisted records.
_EXECUTABLE_LIKE_MAGIC = frozenset(EXECUTABLE_MAGIC) | {'LNK'}

# Narrower than file_inspect.EXECUTABLE_EXTENSIONS on purpose: `.js` and `.dll` appear
# in the manifest of practically every legitimate software distribution and
# web-project archive, so including them would make this signal fire on everything.
...
# Patterns must never cross a newline: the sample is newline-joined, so a match
# spanning two unrelated strings is a coincidence, not an observation.
_CONTENT_RULES = (
    ('reverse_shell', (
        r'/dev/(?:tcp|udp)/',
    ...
    # `IsDebuggerPresent` is deliberately NOT here: the MSVC CRT imports it, so at 0.25
    # plus the two ambient signals every packed installer already earns, it would push
    # a large share of legitimately-built Windows binaries over the suspicious floor.
    ('anti_analysis', (
...
# finditer rather than search: a pure-hex run of the same length (a SHA-256 digest, a
# GUID table) matches the same character class and is not a blob.
_BASE64_RUN_RE = re.compile(r'[A-Za-z0-9+/]{60,}={0,2}')
_HEX_ONLY_RE = re.compile(r'[0-9a-fA-F]+\Z')

# Anchored so an address cannot be carved out of a longer run: without the boundaries
# the Base58 alternation matches constantly inside packed data, exactly the population
# where this signal does the most damage. `+`, `/` and `=` are in the boundary class
# too - a base64 blob is one long run punctuated by exactly those three, so omitting
# them would let a certificate be carved into a "Base58 address" at every `/`.
...
# lnk_parse owns the 30 key names; they are imported rather than retyped so a rename
# there cannot silently stop matching here. test_file_rules.py asserts this is a total
# partition of lnk_parse.SIGNAL_LABELS, forcing a weighting decision on each new key.
LNK_ROLLUPS = {
...
# 'skipped' (no API key), 'error' and 'unavailable' are all *not* answers: a source
# never asked, or that failed, says nothing about the file.
...
def _reputation_answered(reputation):
    ...
    Accepts `hash_service`'s shape (`sources[name]['status']`), the per-source
    accounting shape (`state`), and an explicit `answered` override. Anything
...
def score(record):
    """Never raises on a bad record. ``level`` is ``unknown`` when no reputation source
    answered and nothing structural reached a band - never a false ``benign``.
    """
    ...
    # Neither executable nor archive: PDF, RTF, legacy Office, image. A second program
    # inside one has no benign explanation the way an installer's payload does.
